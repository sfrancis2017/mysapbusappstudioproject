...
{Id:['ciA',720,,'isNamedArg'],'':714},
{
Id:[715,734],'#':'Id','(':'Id','*':'Id','+':'Id','-':'Id',':':'Id','?':'Id',Id_all:'Id',Number:'Id',String:'Id',QuotedLiteral:'Id',
DeleteStarFromSet:['c',731],
'':731
},
{',':['c',716,,,,'nextFunctionArgument'],'':717},
{
Id:[715,734],'#':'Id','(':'Id','*':'Id','+':'Id','-':'Id',':':'Id','?':'Id',not:'Id',case:'Id',cast:'Id',null:'Id',true:'Id',false:'Id',Id_all:'Id',Number:'Id',String:'Id',exists:'Id',QuotedLiteral:'Id',
DeleteStarFromSet:['c',715],
')':['g',717,1,'atRightParen'],order:')',
},
{order:['ck',718],'':731},
['mk',719,'by'],
[731,754],
...
case'*':case'+':case'-':case'/':case'<':case'=':case'>':case'?':case'!=':case'<=':case'<>':case'>=':case'in':case'is':case'or':case'||':case'and':case'not':case'like':case'between':if(this.expression(_={ expr: expr },580,652)){e=_.expr; Object.assign( e.location || {}, expr.location ); art.value = this.attachLocation( e ) }continue
...
case'*':case'+':case'-':case'/':case'<':case'=':case'>':case'?':case'!=':case'<=':case'<>':case'>=':case'in':case'is':case'or':case'||':case'and':case'not':case'like':case'between':if(this.expression($,682,652)){this.queryOnLeft(0,'expr')}continue;
...
case'Id':if(this.gc(714,'isNamedArg')&&this.ciA(720,'paramname'))id=this.lb();continue
default:this.s=714;continue
}
case 714:switch(this.l()){
case'Id':case'#':case'(':case'*':case'+':case'-':case':':case'?':case'Id_all':case'Number':case'String':case'QuotedLiteral':if(this.funcExpression(_={},715)){expr=_.expr; $.pathStep.args.push( expr ); }continue
case'DeleteStarFromSet':this.c(731);continue
default:this.s=731;continue
}
case 715:switch(this.l()){
case',':if(this.c(716)){this.nextFunctionArgument()}continue;
default:this.s=717;continue
}
case 716:switch(this.lk()){
case'Id':case'#':case'(':case'*':case'+':case'-':case':':case'?':case'not':case'case':case'cast':case'null':case'true':case'false':case'Id_all':case'Number':case'String':case'exists':case'QuotedLiteral':if(this.funcExpression(_={},715)){expr=_.expr; $.pathStep.args.push( expr ); }continue
case'DeleteStarFromSet':this.c(715);continue
case')':case'order':this.lP()&&this.ec('atRightParen')&&this.g(717);continue
default:this.ei();continue
}
...
case 719:if(this.orderByClauseAsXpr({outer:expr.args},731)){ this.attachLocation( expr ); }continue
case 720:switch(this.l()){
case':':if(this.c(721)){ $.pathStep.args = this.createDict( open ); $.pathStep.$syntax = ':'; }continue
case'=>':if(this.c(726)){ $.pathStep.args = this.createDict(); }continue
default:this.e();continue
}
case 721:if(this.expression(_={},722)){expr=_.expr; this.addNamedArg( $.pathStep, id, expr ); }continue
case 722:switch(this.l()){
case',':this.c(723);continue
default:this.s=731;continue
}
case 723:switch(this.l()){
case'Id':if(this.ciA(724,'paramname'))id=this.lb();continue
case')':this.s=731;continue
default:this.e();continue
}
case 724:this.m(725,':');continue
case 725:if(this.expression(_={},722)){expr=_.expr; this.addNamedArg( $.pathStep, id, expr ); }continue
case 726:if(this.expression(_={},727)){expr=_.expr; this.addNamedArg( $.pathStep, id, expr ); }continue
case 727:switch(this.l()){
case',':this.c(728);continue
default:this.s=731;continue
}
case 728:switch(this.l()){
case'Id':if(this.ciA(729,'paramname'))id=this.lb();continue
case')':this.s=731;continue
default:this.e();continue
}
case 729:this.m(730,'=>');continue
case 730:if(this.expression(_={},727)){expr=_.expr; this.addNamedArg( $.pathStep, id, expr ); }continue
