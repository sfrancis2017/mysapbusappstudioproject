...
if (typeof process.env.HDI_DEPLOY_OPTIONS !== 'undefined') {
  ...
  for (const option in options) {
    if (option === 'auto_undeploy') {
      ...
      args.translateJSONEnvBooleanOptionToOption(logger, options, name, option, process.argv);
    } else if (option === 'delta_detection') {
      args.translateJSONEnvBooleanOptionToOption(logger, options, name, option, process.argv);
    } else if (option === 'info') {
      args.translateJSONEnvStringArrayOptionToOption(logger, options, name, option, process.argv);
    } else if (option === 'root') {
...
for (let i = 2; i < process.argv.length; ++i) {
  ...
  if (arg === '-h' || arg === '--help') {
    const help = [
      ...
      '                                  by default, delta detection is done by local SHA256 calculation',
      '',
      "        --[no-]delta-detection    [don't] perform delta detection.",
      '                                  by default, delta detection is set to true',
      '',
  ...
  } else if (arg === '--delta-detection') {
    opt.deltaDetection = true;
  } else if (arg === '--no-delta-detection') {
    opt.deltaDetection = false;
  } else if (arg === '--root') {
    ++i;
...
// Logger sets it's client value to hdb if the option is set
logger.determineClient();
...
function getClientVersion (client) {
  const client_module= require(`${client}/package.json`);
...
function showVersionAndOtherInformation (cb) {
  try {
    ...
    if (logger.getClientValue() === logger.hdb) {
      const hdb_version = getClientVersion(logger.hdb);
