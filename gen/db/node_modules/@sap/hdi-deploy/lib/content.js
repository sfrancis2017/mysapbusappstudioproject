...
/**
 * Get a ReadStream of the given file or a Buffer of its modified content in special cases.
 *
 * @param {String} file Filename
 * @param {any} services Services
 * @param {Boolean} stripCRFromCSV True or false
 * @returns {Buffer|ReadStream}
 */
function deployFileContent (file, services, stripCRFromCSV) {
  ...
  if (stripCRFromCSV) {
    ...
    if (fileExtension.toLowerCase() === '.csv') {
      ...
      const replacedContent = Buffer.from(content.filter(byte => byte !== 13)); // 13 is the ASCII code for '\r'
...
class Content {
  ...
  constructor (root, services, workingSet, deployDirs, pathFilter, stripCRFromCSV, excludeFilter) {
    ...
    try {
      /*
       * An array of deployable content. The first element of each nested array is a string representing the file path as deployed,
       * and the second element is either a ReadStream or a Buffer containing the file's content.
       */
