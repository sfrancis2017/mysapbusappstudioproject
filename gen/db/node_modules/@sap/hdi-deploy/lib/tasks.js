...
const { calculateHashFromStream, callbackTimeout, getUniqueAndDuplicatesByKey, getDeployOrUndeployFiles } = require('./utils');
...
/**
 * Classifies files by their modification status.
 *
 * @param {string[]} deployedLocalFiles - Array of file paths to classify.
 * @param {Object[]} listDeployedResult - Array of deployed file objects with `path` **as deployed** and `sha256` properties.
 * @param {Array} deployContent - Array of file content arrays, where each array contains a file path **as deployed** and its content as ReadStream or Buffer.
 * @param {Object} logger - Logger object for logging trace messages.
 * @param {function} callback - Callback function to be called with the results.
 */
function classifyFilesByModificationStatus (deployedLocalFiles, listDeployedResult, deployContent, logger, callback) {
  const modifiedFiles = [];
  const unmodifiedFiles = [];

  async.each(deployedLocalFiles, (currentFilePath, cb) => {
    const file = listDeployedResult.find(file => file.path === currentFilePath);
    const deployFileContent = deployContent.find(file => file[0] === currentFilePath);

    if (file && deployFileContent) {
      if (deployFileContent[1]) {
        let stream;
        if (Buffer.isBuffer(deployFileContent[1])) {
          stream = Readable.from(deployFileContent[1]);
        } else if (deployFileContent[1] instanceof fs.ReadStream) {
          stream = fs.createReadStream(deployFileContent[1].path);
        } else {
          logger.warn(`Unknown content type found for file "${currentFilePath}". ${typeof deployFileContent[1]}`);
          return cb();
        }
        calculateHashFromStream(stream, (err, sum) => {
          if (err) {
            return cb(err);
          }
          if (sum === file.sha256) {
            unmodifiedFiles.push(currentFilePath);
          } else {
        ...
        logger.warn(`No content found for file "${currentFilePath}".`);
        return cb();
      }
    } else {
      logger.warn(`File "${currentFilePath}" could not be processed because of missing information.`);
      logger.trace(`File not found: "${currentFilePath}"`);
      logger.trace(`Deployed files: ${JSON.stringify(listDeployedResult.map(file => file.path), null, 2)}`);
      logger.trace(`Files to deploy: ${JSON.stringify(deployContent.map(file => file[0]), null, 2)}`);
      return cb();
    }
  }, (err) => {
    if (err) {
...
class DeployTask {
  ...
  connect () {
    ...
    if (logger.getClientValue() !== logger.hdb && logger.getHanaClientTrace()) {
  ...
  optimiseSynchronize () {
    ...
    tasks.push(function (callback) {
      async.waterfall([
        ...
        (result, innerCB) => {
          const {addedLocalFiles, deletedLocalFiles, deployedLocalFiles} = prepareSync(result.results, this.content);
          ...
          classifyFilesByModificationStatus(deployedLocalFiles, result.results, this.content.deployContent, this.logger, innerCB);
        },
  ...
  make () {
    ...
    tasks.push(function (callback) {
      async.waterfall([
        ...
        (result, innerCB) => {
          if (this.options.deltaDetection === false) {
            this._deployFiles = getDeployOrUndeployFiles('deploy', this.options, this.content);
            this._undeployFiles = getDeployOrUndeployFiles('undeploy', this.options, this.content);
            this.logger.log(`${this._deployFiles.length} files are scheduled for deploy based on explicit specification`);
            this.logger.log(`${this._undeployFiles.length} files are scheduled for undeploy based on explicit specification`);
